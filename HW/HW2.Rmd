...
# lines(Y.grid, true.density, col = 2, lty = 2, lwd = 2)
# legend(0, .1,  c("True Density of Y", "MC Simulation of Y"), lty=c(2,1), col=c("red", "black"))
...
legend(4, .1,  c("True Density of Y", "MC Simulation of Y"), 
       lty=c(2,1), col=c("red", "black"))


hist(Y, prob = TRUE, xlab = "Y", ylab = "p(Y)", xlim=c(-10,5), ylim=c(0.06, 0.15), main="Zoomed Histogram of Y")

lines(density(Y), lwd = 2)

lines(Y.grid, true.density, col = 2, lty = 1, lwd = 1)

legend(-3, .15,  c("True Density of Y", "MC Simulation of Y"), 
       lty=c(2,1), col=c("red", "black"))

```

The above plots show the true density of $Y$ (red) and the Monte Carlo simulation resluts (black). The histogram plot shows the probability distribution of $Y$. It is plain to see that the true density of $Y$ and the Monte Carlo simulation reults produce nearly identical results. The table below summerizes the results from this experiment. The final column of the table shows the difference between the true values of $Y$ and the Monte Carlo simulation. This indicates that the Monte Carlo simulation technique works very well in this case.  

\bigskip

\begin{tabular}{ c c c c c }
& Initial Values & True Values & MC Simulation (10,000 iter) & $\Delta_{\cdot\cdot\cdot}$ \\ \hline \hline \\
$\mu_1$ & `r mu1 ` & . & . & . \\
$\mu_2$ & `r mu2 ` & . & . & . \\
$\sigma_1^2$ & `r sig2.1 ` & . & . & . \\
$\sigma_1^2$ & `r sig2.2 ` & . & . & . \\
...
$\Delta_{\sigma^2}=\sigma^2-mc.\sigma^2$ & . & . & . & `r sig.sq- Y.var ` \\
\end{tabular}
...
for(i in 1:N){
  X2.samp = rnorm(N, mu2, sqrt(sig2.2))
  dY[i] = mean(dnorm(Y.grid[i]- X2.samp, mu1, sqrt(sig2.1)))
}
lines(Y.grid, dY, col = "blue", lty = 5)
