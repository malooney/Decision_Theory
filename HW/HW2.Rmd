...
Assuming ${Y_i}\mathop  = \limits^{iid} Bernoulli(p)$, then we can say that $Y|p \sim Binomial(n,p)$ and we can obtain the liklehood function as:
...
Letting $k={\sum\nolimits_{i = 1}^n {{y_i}} }$ we obtain the log liklehood as:

$$\log L(p|Y) = k\log (p) + (n - k)\log (1 - p)$$

The next step in the derivation of the jeffreys Prior is to calculate the Fisher information matirx. We are told the data is iid and we can see that the log liklehood function is twice differentiable which allows us to use an alternative form of the Fisher information matrix. Further, calculation is vastly simplified since our liklehood function is low dimensinal (single paramater). The Fisher information is given by the following:

$${\bf{I}_{j,k}}(\boldsymbol{\theta}) =  - \EX_{\bf{y}|\theta}\left[ {{{{\partial ^2}\log L(\bf{y}|\boldsymbol{\theta})} \over {\partial {\theta _j}\partial {\theta _k}}}} \right]$$
